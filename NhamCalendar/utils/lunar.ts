...
export const getMonthCanChi = (year: number, month: number) => {
  ...
  let startCan = 0;
  if (yearCanIndex === 0 || yearCanIndex === 5) startCan = 2; // Bính
  else if (yearCanIndex === 1 || yearCanIndex === 6) startCan = 4; // Mậu
  else if (yearCanIndex === 2 || yearCanIndex === 7) startCan = 6; // Canh
  else if (yearCanIndex === 3 || yearCanIndex === 8) startCan = 8; // Nhâm
  else if (yearCanIndex === 4 || yearCanIndex === 9) startCan = 0; // Giáp
  return `${cans[(startCan + month - 1) % 10]} ${chis[(month - 1) % 12]}`;
};

// 3. Tính Can Chi của Ngày (Dựa trên Julian Day)
...
export const getFullLunarInfo = (date: Date): FullLunarInfo => {
  ...
  // Lấy các giá trị Âm lịch cơ bản từ thư viện
  ...
  return {
    ...
    lunarMonth: lMonth,
    lunarYear: lYear,
    // Gắn trực tiếp các hàm tự tính toán vào để không bao giờ bị rỗng
    canDay: getDayCanChi(date),
    canMonth: getMonthCanChi(lYear, lMonth),
    canYear: getYearCanChi(lYear),
